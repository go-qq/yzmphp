<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:94:"D:\phpStudy\PHPTutorial\WWW\1\atpdemo\tp_1\public/../application/xgadmin\view\index\data2.html";i:1533636318;s:96:"D:\phpStudy\PHPTutorial\WWW\1\atpdemo\tp_1\public/../application/xgadmin\view\public\layout.html";i:1533866265;}*/ ?>
<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<script>
	...
	function del (obj,id) {
		...
		//发生sql语句
		$.post("<?php echo url('ajax_del2'); ?>",{id:id},function  (data) {
				if (data.statu==200) {
	...
	function delAll2 () {
	    ...
	    str=arr.join(',',arr);
	    $.post("<?php echo url('ajax_delAll2'); ?>",{str:str},function  (data) {
	        if (data==arr.length) {
    ...
    function status (obj,id,val) {
        if (val) {
            $.post("<?php echo url('ajax_statu'); ?>",{id:id,statu:0},function  (data) {
                   if (data==1) {
                        $(obj).html('未回访');
            ...
            $.post("<?php echo url('ajax_statu'); ?>",{id:id,statu:1},function  (data) {
                if (data==1) {
                    $(obj).html('已回访');
